...
write_csv(datosFinal, file = "datos/datosFinal.csv")
...
#El modelo informa las probabilidades de FALLECIDO con las variables de entrada, información de las estadísticas a priori. Para probarla se utilza los datos de test con predict():
...
table(test.data$FALLECIDO, clase)
...
###### **Una vez aplicado el modelo a nuestro conjunto de entrenamiento podemos apreciar como la edad de la persona influye notoriamente en la probabilidad de muerte por covid, así como el hecho de que hayan estado internados y/o en cuidados intensivos.**

###### **Por último y viendo la matriz de confusión resultante al aplicar el modelo al resto de los datos podemos ver que nos da una eficacia del 97%, y un intervalo de confianza entre 96% al 97%.**

___

<em>Como conclusión del trabajo podemos comentar que nos ha servido fundamentalmente para entender el ciclo de vida de un proyecto de datos con machine learning, así como el flujo de trabajo necesario para llevarlo a cabo. Entendemos nuestra entrega como una primera iteración, en la cual se aprecian las dificultades que nos encontramos al avanzar con el trabajo a la vez que fuimos aprendiendo cómo debería desarrollarse.</em>